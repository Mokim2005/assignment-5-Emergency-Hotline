...
document.querySelectorAll(".fa-phone").forEach((phoneIcon) => {
    ...
    callBtn.addEventListener("click", (e) => {
        ...
        const h1s = Array.from(card.querySelectorAll("h1")).map(h => h.innerText.trim()).filter(Boolean);
        ...
        historyItem.className = "history-item flex justify-between items-center p-2 border-b bg-white";
...
document.querySelectorAll(".fa-copy").forEach((copyIcon) => {
    ...
    copyBtn.addEventListener("click", (e) => {
        ...
        navigator.clipboard.writeText(serviceNumber).then(() => {
            ...
            alert(Copied! Number ${serviceNumber} copied to clipboard.);
        });
    });
});
